...
def register_api_routes(app):
    ...
    def get_hosts_data():
        ...
        with get_db(app.config) as db:
            ...
            downtime_allotment = db.execute('SELECT value FROM settings WHERE key = "default_downtime_allotment"').fetchone()[0]
        ...
        return jsonify({'hosts_list': hosts_list, 'downtime_allotment': downtime_allotment})
    ...
    def system_info():
        ...
        try:
            ...
            with get_db(app.config) as db:
                ...
                unmonitored_hosts_count = db.execute('SELECT COUNT(*) FROM unmonitored_hosts').fetchone()[0]
            ...
            return jsonify({
                ...
                'unmonitored_hosts_count': unmonitored_hosts_count,
    ...
    @app.route('/api/downtime_allotment', methods=['POST'])
    def update_downtime_allotment():
        try:
            data = request.get_json()
            allotment = data.get('downtime_allotment')

            if allotment is None:
                return jsonify({'error': 'Missing downtime_allotment parameter'}), 400

            with get_db(app.config) as db:
                # Update the default allotment in settings
                db.execute('''
                    UPDATE settings
                    SET value = ?
                ...
                # Update all hosts that have no allotment or zero allotment
                # Note: We don't update last_allotment_reset here
                db.execute('''
                    UPDATE hosts
                    SET downtime_allotment = ?
                    WHERE downtime_allotment IS NULL
                       OR downtime_allotment = 0
                ''', (allotment,))
    ...
    @app.route('/api/downtime_allotment', methods=['GET'])
    def get_downtime_allotment():
        try:
            with get_db(app.config) as db:
                # Get the downtime allotment from settings
                allotment = db.execute('SELECT value FROM settings WHERE key = ?', ('default_downtime_allotment',)).fetchone()
                if allotment:
                    return jsonify({'downtime_allotment': allotment[0]})
                else:
                    return jsonify({'error': 'Default allotment not found'}), 404

            return jsonify({'success': True, 'downtime_allotment': allotment[0]})
        except Exception as e:
            return jsonify({'error': str(e)}), 500